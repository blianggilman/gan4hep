...
class CGAN():
    ...
    def train(self, train_truth, epochs, batch_size, test_truth, log_dir, evaluate_samples_fn, xlabels,
        ...
        print("HERE 1 !!",test_truth.shape)
        print("HERE 1 !!",noise.shape)
        ...
        with tqdm.trange(epochs, disable=self.disable_tqdm) as t0:
            for epoch in t0:
                ...
                noise = np.random.normal(loc=0., scale=1., size=(train_truth.shape, self.noise_dim))
                print("HERE!!",train_truth.shape)
...
if __name__ == '__main__':
    ...
    train_in, train_truth, test_in, test_truth, xlabels = convert_g4_data()
